...
>>>>>>> revert: rm quantity & food filter
=======
import React, { useState } from "react";
...
=======
  ...
  const [activityFilter, setActivityFilter] = useState<string>(
    props.defaultFilters.activity
  );
  const [nutritionFilter, setNutritionFilter] = useState<string>(
    props.defaultFilters.nutrition
  );
...
<<<<<<< HEAD
  ...
  const handleFilterChange = (filter: string, activity: boolean) => {
    if (activity) setActivityFilter(filter);
    else setNutritionFilter(filter);
